/// HM4 using Array of Student Node Structures
// This file will contain all the code for Homework 4 (Min Heap implemented via Enhanced Array).
...
public: // Publically supported methods
    ...
    { //  1. Normal Constructor.  Create ESA of this size.  May need to be resized later
        ...
        cnum = 0;
        tp = NULL; // Initialize with no nodes currently in array
    }

    // SnEsa ();                // 2. Default Constructor <Commented out>

    SnEsa(SnEsa &sep)
    { // 3. Copy Constructor  (Create new array and copy Student Nodes, not Student ptrs or Student Node ptrs)
        cnum = sep.cnum;
        cmz = sep.cmz;
    ...
    {
        if ((idx < 0) || (idx > cnum))
            return (-1); // Bad index (note:  Ok if idx == cnum, it's an append)
        if (cnum == cmz)
        {                                       // Whoa!  Array at max size now ... realloc needed
            tp = new studNode[cmz = (2 * cmz)]; // Allocate twice the current max size and reset max
            cout << "Doubling Array size to " << cmz << endl
                 << endl;
            for (int i = 0; i < cnum; i++)
                tp[i] = snap[i]; // Copy all existing Student node ptrs
            delete[] snap;       // Free old array
            snap = tp;           // Set sap to start of new array
        }
        for (int i = cnum - 1; i >= idx; i--)
            snap[i + 1] = snap[i]; // Move following ptrs back one
    ...
    int remove(int idx)
    {
        ...
        for (int i = idx; i < cnum - 1; i++)
            snap[i] = snap[i + 1]; // Pull forward all subsequent elements
        return (--cnum);
    }
...
private:
    ...
    { // Perculate new element up from starting position.
        ...
        if (pNode.priority <= cNode.priority)
            return (-1); // If swap not needed, done.
    ...
    int percolDown(int sidx)
    { // If needed, swap parent node and lower priority child node
      // If swap occurs, return child as new parent or -1 if no swap
        int hsz, lidx, ridx;

        if (sidx == -1)
            return (-1); // Perculate Down bottomed out at lower level
    ...
    public:
        ...
        StudMinHeap(StudMinHeap & hold)
        {                       // Equate this to an existing Heap (can use private parts directly)
        ...
        ~StudMinHeap()
        {                  // Destruct the Heap (free ESA space)
        ...
        int push(unsigned int pri, Student *sp)
        {                            // Add new Student to heap at indicated priority
        ...
        Student *pop()
        {                        // Pop highest priority node from Min Heap (in root position) & rebalance lower nodes.
            ...
            if (!(sp = snap->get(0).studp))
                return (sp); // A Zero student pointer in Root Node means Heap is empty
        ...
        { // Return number of Students at specified priority
